...
import { InfoLine, CreditsBlock, NotActive, Identifier } from '../../../components/data'
...
function BroadcastForm() {
  ...
  const [rate, setRate] = useState<{ data: Rate | null; loading: boolean; error: unknown }>({
    data: null,
    loading: true,
    error: null
  })
  ...
  const handleInputChange = (value: string) => {
    setInput(value)
    if (verify || decoded || errorText || state === STATE.UNSIGNED || state === STATE.SUCCESS)
      reset()
  }
  ...
  const verifyHex = async (rawHex: string) => {
    ...
    try {
      ...
      const [decodedResult, verifyResult] = await Promise.all([
        Api.decodeTx(base64).catch(e => {
          console.warn('decode failed:', e)
          return null
        }),
        Api.verifyTransaction(payload).catch(e => {
      ...
      if (!parsed.isSigned) {
        if ((verifyResult as VerifyResult | null)?.result === 'error') {
          const fixableBySigning = SIGN_FIXABLE_ERRORS.has(
            String((verifyResult as VerifyResult).error)
          )
        ...
        setState(
          (verifyResult as VerifyResult | null)?.result === 'ok'
            ? STATE.VERIFIED_OK
            : STATE.VERIFIED_FAIL
        )
      }
  ...
  const handleSignPrivateKey = async () => {
    ...
    try {
      ...
      const signedTx = (await activeSigner.sign(tx)) as {
        hex: () => string
        hash: (b: boolean) => string
      }
  ...
  const handleSignAndBroadcastExtension = async () => {
    ...
    const activeSigner = signerCtl.isConnected ? signerCtl.signer : await signerCtl.connect()
    ...
    try {
      ...
      const signedTx = (await activeSigner.signAndBroadcast(tx)) as {
        hex: () => string
        hash: (b: boolean) => string
      }
  ...
  const getPrimaryAction = () => {
    if (state === STATE.BROADCASTING || state === STATE.WAITING) {
      return {
        label: 'Broadcast',
        isLoading: true,
        loadingText: state === STATE.WAITING ? 'Waiting…' : 'Broadcasting…',
        isDisabled: true
      }
    }
      ...
      if (signerCtl.method === SignerMethod.EXTENSION) {
        if (!signerCtl.isConnected) {
          return {
            ...
            onClick: () => signerCtl.connect(),
            isLoading: signerCtl.isConnecting,
            loadingText: 'Connecting…'
          }
        }
        return {
          label: 'Sign & Broadcast',
          onClick: handleSignAndBroadcastExtension,
          isLoading: state === STATE.SIGNING,
          loadingText: 'Awaiting popup…'
        }
      }
      return {
        label: 'Sign',
        onClick: handleSignPrivateKey,
        isLoading: state === STATE.SIGNING || signerCtl.isConnecting,
        loadingText: 'Signing…',
        isDisabled: !wif.trim()
      }
    }
          ...
          <div className={'BroadcastForm__SignPanel'}>
            ...
            {signerCtl.method === SignerMethod.PRIVATE_KEY && (
              <PrivateKeyForm
                ...
                identityIdPlaceholder={
                  detectedOwnerId ? 'Identity ID (detected from tx)' : 'Identity ID (optional)'
                }
              />
            )}
          ...
          <div className={'BroadcastForm__ButtonMessage'}>
            {verify?.result === 'error' && state !== STATE.UNSIGNED && (
              <span className={'BroadcastForm__ButtonMessage--error'}>
                {explainConsensusError(
                  verify.error,
                  typeof verify.code === 'number' ? verify.code : Number(verify.code)
                )}
              </span>
            )}
            {errorText && (
      ...
      <div className={'BroadcastForm__Section BroadcastForm__Section--Json'}>
        <Tabs variant={'line'} isLazy className={'BroadcastForm__Tabs'}>
          ...
          <TabPanels className={'BroadcastForm__TabPanels'}>
            <TabPanel
              className={'BroadcastForm__TabPanel BroadcastForm__TabPanel--Variant TransactionPage'}
            >
              <div className={'BroadcastForm__DecodedMeta'}>
                ...
                <InfoLine
                  ...
                  value={
                    decoded?.typeString ? (
                      <TypeBadge type={decoded.typeString} />
                    ) : (
                      <NotActive>—</NotActive>
                    )
                  }
                ...
                <InfoLine
                  ...
                  value={
                    hasStatus ? (
                      <TransactionStatusBadge status={statusValue} />
                    ) : (
                      <NotActive>—</NotActive>
                    )
                  }
                ...
                <InfoLine
                  ...
                  value={
                    decoded?.identityNonce != null ? (
                      decoded.identityNonce
                    ) : (
                      <NotActive>—</NotActive>
                    )
                  }
                ...
                <InfoLine
                  ...
                  value={
                    decoded?.signaturePublicKeyId != null ? (
                      decoded.signaturePublicKeyId
                    ) : (
                      <NotActive>—</NotActive>
                    )
                  }
