      ...
      <ModalContent className='InfoBlock InfoBlock--Gradient'>
        ...
        <ModalBody>
          ...
          <Box mt={4} fontSize='12px' fontFamily='mono' color='var(--chakra-colors-gray-250)'>
            Owner: <Box as='span' color='var(--chakra-colors-white-100)' wordBreak='break-all'>{signerIdentityId || '—'}</Box>
          </Box>
          ...
          <Box
            ...
            lineHeight='150%'
          >
            Tip: try on testnet first. Once deployed, supply and rule changes are restricted
            by the configured authorization. Peer-to-peer marketplace is not supported by the
            protocol yet — only direct purchase from the issuer (set later).
          </Box>
        </ModalBody>
        <ModalFooter gap={2}>
          <Button variant='ghost' size='sm' onClick={onClose} isDisabled={isDeploying}>Cancel</Button>
          <Button
            variant='blue'
            size='sm'
            minW='160px'
