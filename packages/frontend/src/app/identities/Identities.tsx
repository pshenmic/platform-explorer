...
function Identities ({ defaultPage = 1, defaultPageSize, defaultShowAll = false }: IdentitiesProps) {
  ...
  const fetchData = (page: number, count: number, includeMasternodes: boolean, currentFilters: Record<string, unknown>) => {
      ...
      >
        <div className={'IdentitiesPage__Controls'}>
          ...
          <NetworkStatsInline
            ...
            items={[{ label: 'Total', value: formatFullNumber(total) as string | number, loading: identities.loading }]}
          />
        ...
        {!identities.error
          ? !identities.loading
              ? <IdentitiesList
                  ...
                  sort={{ order_by: (filters.order_by as string) || 'balance', order: ((filters.order as string) || 'desc') as 'asc' | 'desc' }}
                  page={currentPage}
                  onSortChange={(next) => {
                    setFilters(next)
                    setCurrentPage(0)
