...
import type {
  ContestedResource as ContestedResourceType,
  LoadableState,
  PaginatedResultSet,
  Rate,
  Vote
...
const tabs = ['all_votes', 'towards_identity', 'abstain', 'locked'] as const
...
function ContestedResource({ resourceValue }: ContestedResourceProps) {
  ...
  const [contestedResource, setContestedResource] = useState<Refreshable<ContestedResourceType>>({
    data: {} as ContestedResourceType,
    loading: true,
    error: false,
    refreshing: true
  })
  ...
  const [activeTab, setActiveTab] = useState(
    tabs.indexOf(defaultTabName as (typeof tabs)[number]) !== -1
      ? tabs.indexOf(defaultTabName as (typeof tabs)[number])
      : 0
  )
  const [rate, setRate] = useState<Refreshable<Rate>>({
    data: {} as Rate,
    loading: true,
    error: false,
    refreshing: true
  ...
  useEffect(() => {
    ...
    const isInitialLoad =
      contestedResource.data == null || Object.keys(contestedResource.data).length === 0
  ...
  useEffect(() => {
    ...
    Api.getContestedResourceVotes(
      resourceValue,
      (votes.props as PaginatedProps).currentPage + 1,
      pageSize,
      'desc'
    )
  ...
  useEffect(() => {
    ...
    Api.getContestedResourceVotes(
      resourceValue,
      (towardsIdentityVotes.props as PaginatedProps).currentPage + 1,
      pageSize,
      'desc',
      { choice: 0 }
    )
    ...
    resourceValue,
    (towardsIdentityVotes.props as PaginatedProps).currentPage,
    towardsIdentityVotes.refreshing
  ])
  ...
  useEffect(() => {
    ...
    Api.getContestedResourceVotes(
      resourceValue,
      (abstainVotes.props as PaginatedProps).currentPage + 1,
      pageSize,
      'desc',
      { choice: 1 }
    )
  ...
  useEffect(() => {
    ...
    Api.getContestedResourceVotes(
      resourceValue,
      (lockedVotes.props as PaginatedProps).currentPage + 1,
      pageSize,
      'desc',
      { choice: 2 }
    )
  ...
  return (
    <PageDataContainer className={'ContestedResource'} title={'Contested Resource info'}>
      <ContestedResourceTotalCard
        refresh={refreshAfterVote}
        isPollingAfterVote={isPollingAfterVote}
        contestedResource={contestedResource as never}
        rate={rate as never}
      />
          ...
          <TabList>
            <Tab>
              ...
              {votes.data?.resultSet && (
                <span
                  className={`Tabs__TabItemsCount ${votes.data?.resultSet?.length === 0 ? 'Tabs__TabItemsCount--Empty' : ''}`}
                >
            ...
            <Tab>
              ...
              {towardsIdentityVotes.data?.resultSet && (
                <span
                  className={`Tabs__TabItemsCount ${towardsIdentityVotes.data?.resultSet?.length === 0 ? 'Tabs__TabItemsCount--Empty' : ''}`}
                >
            ...
            <Tab>
              ...
              {abstainVotes.data?.resultSet && (
                <span
                  className={`Tabs__TabItemsCount ${abstainVotes.data?.resultSet?.length === 0 ? 'Tabs__TabItemsCount--Empty' : ''}`}
                >
            ...
            <Tab>
              ...
              {towardsIdentityVotes.data?.resultSet && (
                <span
                  className={`Tabs__TabItemsCount ${lockedVotes.data?.resultSet?.length === 0 ? 'Tabs__TabItemsCount--Empty' : ''}`}
                >
          ...
          <TabPanels>
              ...
              <VotesList
                ...
                pagination={{
                  onPageChange: pagination =>
                    paginationHandler(setTowardsIdentityVotes, pagination.selected),
                  pageCount:
                    Math.ceil((towardsIdentityVotes.data?.pagination?.total ?? 0) / pageSize) || 1,
            ...
            <TabPanel position={'relative'}>
              <VotesList
                ...
                pagination={{
                  onPageChange: pagination =>
                    paginationHandler(setAbstainVotes, pagination.selected),
            ...
            <TabPanel position={'relative'}>
              <VotesList
                ...
                pagination={{
                  onPageChange: pagination =>
                    paginationHandler(setLockedVotes, pagination.selected),
