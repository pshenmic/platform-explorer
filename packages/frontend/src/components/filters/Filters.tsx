...
export const Filters = ({
  ...
  const previousAppliedFilters = useRef<Record<string, unknown>>(
    appliedFilters as Record<string, unknown>
  )

  const applyFilters = useCallback(
    (filtersToApply: FilterState = menuFilters) => {
      const processedFilters = (() => {
        return Object.entries(filtersToApply).reduce<Record<string, unknown>>(
          (result, [key, value]) => {
            const filterKeyConfig = filtersConfig[key]
            if (!filterKeyConfig) return result

            if (
              filterKeyConfig.type === 'multiselect' &&
              (filterKeyConfig.isAllSelected?.(value) ||
  ...
  const handleFilterChange = useCallback(
    (filterName: string, value: FilterStateValue) => {
  ...
  const resetAllAppliedFilters = useCallback(() => {
    const nextDefaultFilters: FilterState = Object.fromEntries(
      Object.keys(filtersConfig).map(key => [key, filtersConfig[key].defaultValue])
    )
    ...
    () =>
      Object.entries(filtersConfig).map(([key, config]) => {
          ...
          case 'daterange':
            content = (
              <FilterGroup title={config?.title}>
                <DateRangeFilter
                  value={
                    (menuFilters[key] as DateRangeFilterValue | undefined) ?? {
                      start: null,
                      end: null
                    }
                  }
        ...
        const activeFilterValue =
          !filtersConfig[key].isAllSelected?.(appliedFilters[key]) && appliedFilters[key]
    ...
    () => (
      <button
        ...
        onClick={
          isMobile
            ? () => {
                menuIsOpen ? handleMenuClose() : handleMenuOpen()
              }
            : undefined
        }
      >
