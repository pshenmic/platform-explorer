...
function ChoiceBadge({ choice, className }: ChoiceBadgeProps) {
  ...
  const info =
    typeof choiceKey === 'string' ? ChoiceInfo[choiceKey as keyof typeof ChoiceInfo] : undefined

  return (
    <div className={`VoteBadge ${className || ''}`}>
      <Badge className={'VoteBadge__Badge'} colorScheme={info?.colorScheme} size={'xs'}>
        {info?.title}
      </Badge>
    </div>
  )
