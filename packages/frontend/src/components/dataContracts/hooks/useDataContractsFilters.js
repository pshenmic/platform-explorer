...
export const useDataContractsFilters = () => {
  ...
  const [isSystem, setIsSystem] = useQueryState('is_system', {
    scroll: false,
    shallow: true
  })
  const [withTokens, setWithTokens] = useQueryState('with_tokens', {
    scroll: false,
    shallow: true
  })
  const [dcMin, setDcMin] = useQueryState('documents_count_min', {
    scroll: false,
    shallow: true
  })
  const [dcMax, setDcMax] = useQueryState('documents_count_max', {
    scroll: false,
    shallow: true
  })
    ...
    if ('documents_count_min' in next) {
      setDcMin(
        next.documents_count_min !== '' && next.documents_count_min != null
          ? String(next.documents_count_min)
          : null
      )
    }
    ...
    if ('documents_count_max' in next) {
      setDcMax(
        next.documents_count_max !== '' && next.documents_count_max != null
          ? String(next.documents_count_max)
          : null
      )
    }
