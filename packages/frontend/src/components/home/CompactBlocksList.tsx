...
export function CompactBlocksList({
  ...
  const footer = moreHref ? (
    <Link href={moreHref} prefetch={false} className={'CompactBlocks__More'}>
      {moreLabel || 'View all'}
    </Link>
  ) : null
        ...
        {Array.from({ length: limit }).map((_, i) => (
          <div className={'CompactBlocks__Row CompactBlocks__Row--skeleton'} key={i}>
            ...
            <span className={'CompactBlocks__HashCell'}>
              <Skeleton w={'72%'} h={'0.8em'} />
            </span>
            <span className={'CompactBlocks__Txs'}>
              <Skeleton w={'40px'} h={'1.05em'} radius={4} />
            </span>
            <span className={'CompactBlocks__Time'}>
              <Skeleton w={'42px'} h={'0.7em'} />
            </span>
          </div>
        ))}
        {footer}
  ...
  return (
    <div className={'CompactBlocks'} {...hoverBind}>
      ...
      {rows.map((block, i) => {
        ...
        const style = isNew ? ({ '--stagger': `${i * 50}ms` } as CSSProperties) : undefined
        return (
          <Link
            key={hash}
            href={`/block/${hash}`}
          ...
          >
            <span className={'CompactBlocks__Height'}>
              ...
              {typeof header?.height === 'number' ? (
                <BigNumber>{header.height}</BigNumber>
              ) : (
                <NotActive />
              )}
            </span>
            ...
            <span className={'CompactBlocks__Time'}>
              {header?.timestamp ? (
                <TimeDelta
                  ...
                  format={'compact'}
                  endDate={new Date(header.timestamp)}
                />
              ) : (
                <NotActive />
              )}
