...
export default function ShieldedPoolCard({ rate, enabled = true }: ShieldedPoolCardProps) {
  ...
  }>({ loading: true, error: false, balance: null, notes: null })
  const [period, setPeriod] = useState<{
    loading: boolean
    in: number
    out: number
    transitions: number
  }>({ loading: true, in: 0, out: 0, transitions: 0 })
  const [flows, setFlows] = useState<{ loading: boolean; buckets: FlowBucket[] }>({
    loading: true,
    buckets: []
  })
    ...
    Api.getShieldedPool()
      .then(res =>
        setPool({
          loading: false,
          error: false,
          balance: res?.poolBalance ?? null,
          notes: res?.notesCount ?? null
        })
      )
    ...
    <Box className={'InfoBlock InfoBlock--NoBorder ShieldedPool'} w={'100%'}>
      ...
      <div className={'ShieldedPool__Body'}>
        {pool.loading ? (
          ...
          <>
            <RateTooltip
              dash={creditsToDash(balanceCredits)}
              rate={rate?.data as { usd: number } | null | undefined}
              placement={'top'}
            >
            ...
            <div className={'ShieldedPool__Spark'}>
              {flows.loading && !buckets.length ? (
                ...
                >
                  ...
                  >
                    <line
                      className={'ShieldedPool__Baseline'}
                      x1={0}
                      x2={SPARK_W}
                      y1={BASELINE}
                      y2={BASELINE}
                  ...
                  {hovered && (
                    <div className={'ShieldedPool__Tip'} style={{ left: `${hover!.xPct}%` }}>
                      <span className={'ShieldedPool__TipDate'}>
                        {hovered.ts ? tipFmt.format(new Date(hovered.ts as string)) : '—'}
                      </span>
                      <span className={'ShieldedPool__TipRow ShieldedPool__TipRow--in'}>
                        In {fmtDash(creditsToDash(hovered.inAmt))} DASH
                      </span>
                      <span className={'ShieldedPool__TipRow ShieldedPool__TipRow--out'}>
                        Out {fmtDash(creditsToDash(hovered.outAmt))} DASH
                      </span>
                    </div>
                  )}
                </div>
              ...
              {!flows.loading && buckets.length >= 2 && (
                <div className={'ShieldedPool__Axis'}>
                  <span>
                    {buckets[0].ts ? tipFmt.format(new Date(buckets[0].ts as string)) : ''}
                  </span>
                  {buckets.length >= 5 && (
                    <span>
                      {buckets[Math.floor(buckets.length / 2)].ts
                        ? tipFmt.format(
                            new Date(buckets[Math.floor(buckets.length / 2)].ts as string)
                          )
                        : ''}
                    </span>
                  )}
                  <span>
                    {buckets[buckets.length - 1].ts
                      ? tipFmt.format(new Date(buckets[buckets.length - 1].ts as string))
                      : ''}
                  </span>
                </div>
              )}
              ...
              <div className={'ShieldedPool__SparkLegend'}>
                <span>
                  <i className={'ShieldedPool__Dot ShieldedPool__Dot--in'} /> shield
                </span>
                <span>
                  <i className={'ShieldedPool__Dot ShieldedPool__Dot--out'} /> unshield
                </span>
                <span className={'ShieldedPool__SparkWindow'}>
                  {PRESETS[presetIdx].label.toLowerCase()}
                </span>
              </div>
            </div>
          </>
