...
export default function RateTooltip({
  ...
  if (resolvedUsd == null && typeof resolvedDash === 'number' && rate?.usd)
    resolvedUsd = resolvedDash * rate.usd
      ...
      label={
        <div className={'RateTooltip'}>
          {typeof resolvedDash === 'number' && (
            <div className={'RateTooltip__Dash'}>
              {removeTrailingZeros(Number(resolvedDash).toFixed(8))} Dash
            </div>
          )}
          {typeof resolvedUsd === 'number' && (
            <div className={'RateTooltip__Usd'}>~{roundUsd(Number(resolvedUsd))}$</div>
          )}
        </div>
      }
