    ...
    <div ref={wrapRef} className={`DataList ${className}`.trim()}>
      ...
      >
        {cols.map(c => (
          <div
            key={c.key}
            className={`DataList__HeadCell DataList__HeadCell--${c.align || 'left'}`}
          >
            {c.header}
          </div>
        ))}
      ...
      <div className={'DataList__Body'}>
        {loading ? (
          ...
          items.map((item, i) => {
            ...
            const inner = (
              <div key={key} className={'DataList__Row'} style={{ gridTemplateColumns: template }}>
                {cells}
              </div>
            )
            return rowHref ? (
              <Link key={key} href={rowHref(item)} className={'DataList__RowLink'}>
                {inner}
              </Link>
            ) : (
