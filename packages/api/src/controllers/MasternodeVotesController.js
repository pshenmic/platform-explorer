...
class MasternodeVotesController {
  ...
  getMasternodeVotes = async (request, response) => {
    ...
    const masternodeVotes = await this.masternodeVotesDAO.getMasternodeVotes(timestampStart, timestampEnd, voterIdentity, towardsIdentity, choice, power, query, Number(page ?? 1), Number(limit ?? 10), order)
