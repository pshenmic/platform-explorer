...
class MasternodeVotesController {
  ...
  getMasternodeVotes = async (request, response) => {
    ...
    const masternodeVotes = await this.masternodeVotesDAO.getMasternodeVotes(choice, resourceValue, timestampStart, timestampEnd, voterIdentity, towardsIdentity, power, Number(page ?? 1), Number(limit ?? 10), order)
