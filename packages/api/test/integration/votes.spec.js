...
describe('Epoch routes', () => {
  ...
  describe('searchMasternodeVotes()', async () => {
    it('should return mn vote by tx hash', async () => {
      const [,, { mnVote }] = masternodeVotes.sort((a, b) => a.mnVote.id - b.mnVote.id)

      const { body } = await client.get(`/masternodes/votes/search?query=${mnVote.state_transition_hash}`)
        .expect(200)
        .expect('Content-Type', 'application/json; charset=utf-8')
    ...
    it('should return mn vote by voter identity', async () => {
      ...
      const { body } = await client.get(`/masternodes/votes/search?query=${mnVote.voter_identity_id}`)
        .expect(200)
        .expect('Content-Type', 'application/json; charset=utf-8')
    ...
    it('should return mn vote by voter identity order desc', async () => {
      ...
      const { body } = await client.get(`/masternodes/votes/search?order=desc&query=${mnVote.voter_identity_id}`)
        .expect(200)
        .expect('Content-Type', 'application/json; charset=utf-8')
