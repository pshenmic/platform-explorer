...
impl PostgresDAO {
  pub async fn create_transfer(&self, transfer: Transfer, st_hash: String, sql_transaction: &Transaction<'_>) -> Result<(), PoolError> {
    ...
    sql_transaction.execute(&stmt, &[&amount, &sender, &recipient, &st_hash]).await.unwrap();
