...
impl PSQLProcessor {
    pub async fn handle_st(
        ...
        let bytes = match state_transition.clone() {
            ...
            .unwrap(),
            StateTransition::DocumentsBatch(st) => PlatformSerializable::serialize_to_bytes(
                &StateTransition::DocumentsBatch(st.clone()),
            )
            .unwrap(),
        ...
        match state_transition {
            ...
            StateTransition::DocumentsBatch(_st) => {
                self.handle_documents_batch(_st, st_hash, sql_transaction)
                    .await;
