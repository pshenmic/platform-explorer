...
impl PSQLProcessor {
  pub async fn handle_document_transition(&self, document_transition: DocumentTransition, owner_id: Identifier, st_hash: String, sql_transaction: &Transaction<'_>) -> () {
    ...
    match document_transition {
      DocumentTransition::Transfer(_) => {
        self.dao.assign_document(document, owner_id, sql_transaction).await.unwrap();
      }
      ...
      DocumentTransition::Purchase(_) => {
        ...
        self.dao.assign_document(document.clone(), owner_id, sql_transaction).await.unwrap();
